...
async function hackerone() {
    ...
    for (var i = 0; i <= unique.length - 1; i++) {
        if (global.snippet.isnullval(unique[i].trim()) == false) {
            ...
            domain = global.snippet.replaceallstr(domain,'http://','');
            domain = global.snippet.replaceallstr(domain,'https://','');
            domain = global.snippet.replaceallstr(domain,'.*','');
            domain = global.snippet.replaceallstr(domain,'*.','');
            domain = global.snippet.replaceallstr(domain,'*','');
            ...
            var cfnd = programAr.find(o => (o.name == name) && (o.domain == domain));
            if (cfnd == undefined) {
...
async function save_programs() {
    ...
    var sql = "SELECT * FROM program WHERE (page='" + programAr[indx].page + "') and (domain='" + programAr[indx].domain + "')";
