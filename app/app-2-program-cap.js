...
async function domain_check_1() {

    addlog('targetcheck1');
    var sql = "SELECT * FROM program WHERE (domain != '') and (flag is NULL) limit 1";
    var body2 = await global.outgoing.doRequest(global.queryapi, { query: encodeURIComponent(sql) });
    console.log(':doamincheck-01' + ':::' + body2 + '\n**endlog**\n');
    var fnd = false;
    var res = [];
    try {
        res = JSON.parse(body2.trim());
    ...
    if(fnd == true){
        ...
        var payl = res[0].domain;

        payl = global.snippet.replaceallstr(payl,'http://','');
        payl = global.snippet.replaceallstr(payl,'https://','');
        payl = global.snippet.replaceallstr(payl,'.*','');
        payl = global.snippet.replaceallstr(payl,'*.','');
        payl = global.snippet.replaceallstr(payl,'*','');
        ...
        if (cando == true) {
            try {
                ...
                /*} else {
                    ...
                    var sql = "update program set flag='capok',udate=now() WHERE (id=" + id + ")";
                    var body2 = await global.outgoing.doRequest(global.queryapi, { query: encodeURIComponent(sql) });
                    console.log(':domaincheck-02' + ':::' + body2 + '\n**endlog**\n');
              ...
              //  cupage++;
                ...
                await domain_check_1();

            } catch (ex) {


                var sql = "update program set flag='capnotok',udate=now() WHERE (id=" + id + ")";
                var body2 = await global.outgoing.doRequest(global.queryapi, { query: encodeURIComponent(sql) });
                console.log(':domaincheck-02' + ':::' + body2 + '\n**endlog**\n');
...
(async function main() {
    ...
    Promise.allSettled([domain_check_1()]);
